...
function validate(input) {
  ...
  if (!input.difficulty || input.difficulty > 5 || input.difficulty < 1) {
    alert("Difficulty is required, and it has to be from 1 to 5");
  }
  if (!input.duration || input.duration > 12 || input.duration < 1) {
  ...
  return (
    <div className="createContainer">
      <div className="create1">
        <div>
          <Link to="/home">
            <button className="goback">Back to Home</button>
          </Link>
        </div>
        ...
        <div className="things">
          <form>
            ...
            <div>
              ...
              <input
                ...
                name="difficulty"
                onChange={(ev) => hadleDifficulty(ev)}
              />
            ...
            <div>
              <label>
                Countries:{" "}
                <select className="blank" onChange={(ev) => hadleSelect(ev)}>
                  <option value="" disabled="">
                    -Select Country-
                  </option>
          ...
          <div className="activity4">
            {input.countryId.map(
              (
                el //para borrar paises despues de agregarlos
              ) => (
                <div className="activity5-grid" key={el.name}>
                  {" "}
                  {/* declaro key para que no se rompa */}
                  <h6 className="activity6">{el}</h6>
                  <button
                    className="activity-button"
                    onClick={() => hadleDelete(el)}
                  >
                    x
                  </button>
                </div>
